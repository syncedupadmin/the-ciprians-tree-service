...
export function SiteHeader(){return <header className="nav nav-page"><Link className="logo" href="/" aria-label="The Ciprians Tree Service home"><Image src="/images/team-work.jpg" alt="The Ciprians Tree Service official logo" width={991} height={1054} priority unoptimized/><span className="logo-lockup">The Ciprians<small>Tree Service</small></span></Link><nav className="desktop">{links.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}</nav><a className="call" href={phone}>502-692-8014</a><details className="mobile"><summary>Menu</summary><div>{links.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}<a href={phone}>Call now</a></div></details></header>}
